...
//to test US DMA transfer in descriptor mode conrinuously
INT32 DOCS_DMA_Test3(INT32 ctxt, INT32 mode, INT32 times)
{
    UINT32 Buff_Addr;
    UINT32 Buff_len;
    Dma_Buff_t Dma_Buff[80];
    PDma_Buff_t PDma_Buff;

    PDma_Buff = Dma_Buff;
    //ENable all interrupt
    /*HAL_Reg_Write(DOC_INT_REGISTER_BASE_ADDRESS + 0x4 , 0xFFFFFFFF, \
               DOCS_HAL_REG_INT);*/
    times =30; //intial test
    while(times)
    {
        ...
        PDma_Buff->Addr = Buff_Addr;
        PDma_Buff->Len  = Buff_len;
        PDma_Buff++;
        DOCS_POOL_OSM_GetPkt(&Buff_Addr); //since tlv exceeds size 2k
        times--;
    }//while
    HAL_Dma_Send_Buffer(ctxt,Dma_Buff,63);
